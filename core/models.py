...
class TblInstrumento(models.Model):
    """
    Catálogo opcional de instrumentos (acciones, bonos, fondos, etc.).
    - Puede NO usarse en el ingreso manual (el corredor escribe libremente
      en 'instrumento_text'), pero queda disponible por si más adelante
      se estandariza el maestro de instrumentos.
    """
    ...
    def __str__(self):
        # Ejemplo: "ACCIÓN XYZ (ACC)"
        return f"{self.nombre} ({self.tipo_instrumento})"


class TblTipoIngreso(models.Model):
    """
    Catálogo de tipo de ingreso de la calificación (p.ej. 'Corredor', 'Bolsa').
    - 'prioridad' permite resolver conflictos cuando existen dos fuentes
      para una misma calificación (regla de prevalencia).
    """
    ...
    class Meta:
        ...
        ordering = ["prioridad"]  # se listan de menor a mayor prioridad
...
# =============================================================================
# EVIDENCIA (CARGA MASIVA)
# =============================================================================

class TblArchivoFuente(models.Model):
    """
    Representa el archivo (CSV/XLSX) subido en la carga masiva:
    - 'nombre_archivo' y 'ruta_almacenamiento' sirven para trazar la evidencia
      (en local/S3/etc.)
    - 'usuario' es quién subió el archivo.
    - Relación con TblCalificacion a través de 'archivo_fuente' (opcional).
    """
    ...
    usuario = models.ForeignKey(
        User,
        on_delete=models.PROTECT,      # no borrar usuario si tiene archivos
...
# =============================================================================
# CALIFICACIÓN TRIBUTARIA (MAESTRO)
# =============================================================================

class TblCalificacion(models.Model):
    """
    Entidad principal de negocio (una calificación tributaria).
    Se crea por:
      - Ingreso manual (paso 1: datos base; paso 2: montos por factor 8..37),
      - o importación masiva desde archivo (relación con TblArchivoFuente).

    Notas:
    - 'instrumento_text' guarda el valor libre que ingresa el corredor.
      'instrumento' (FK) es opcional por si luego se normaliza contra el catálogo.
    - 'usuario' guarda el último usuario que creó/modificó la calificación
      (se setea en las vistas al guardar).
    - 'factor_actualizacion' es un multiplicador independiente de los factores 8..37,
      usado si necesitas reexpresar/actualizar valores monetarios (IPC/UF/etc.).
    """
...
# =============================================================================
# FACTORES (DETALLE) 8..37
# =============================================================================

class TblFactorValor(models.Model):
    """
    Detalle por calificación con las posiciones 8..37.
    - 'monto_base'  : monto ingresado por el corredor en la pantalla de montos.
    - 'valor'       : factor calculado como monto_pos / suma_montos(8..19),
                      redondeado a 8 decimales.
    Reglas clave:
    - Se debe garantizar (en la vista/servicio) que la suma de 'valor' para
      posiciones 8..19 sea <= 1.00000000.
    - Una calificación no puede tener más de un registro por 'posicion'
      (unique_together).
    """
...
class TblFactorDef(models.Model):
    posicion = models.PositiveSmallIntegerField(unique=True)  # 8..37
    codigo = models.CharField(max_length=30, blank=True)      # opcional (ej: F8)
    nombre = models.CharField(max_length=255)                 # ej: "Con crédito por IDPC..."
